...
export class RegistracijaComponent implements OnInit {
  ...
  dopuniPoljaUFormi(imeSlike) {
    ...
    this.korisnikServis.registracija(data).subscribe((res) => {
      alert(res["message"]);
      this.ruter.navigate(['']);
    })
  }
  ...
  posaljiZahtevZaRegistraciju() {
    ...
    if (this.nazivOrg != "" || this.adresaOrg != "" || this.matBrOrg != "") {
      if (this.nazivOrg == "" || this.adresaOrg == "" || this.matBrOrg == "") {
        alert("Ukoliko želite da se registrujete kao organizator, popunite polja za " +
    ...
    if (this.slika != null) {
      ...
      this.korisnikServis.posaljiSliku(formData).subscribe((res) => {
        ...
        if (res["originalname"]) { // ako je prosla provjera formata
          this.korisnikServis.proveriVelicinuSlike(res["originalname"]).subscribe((res) => {
            //alert(res["message"]);
            if (res["message"] == "OK") { // ako je prosla provjera velicine slike
